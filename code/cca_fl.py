...
def createDAG(Sf: list, relations: list,F_plus: list, F_minus: list) -> list:
    DAG = []
    forbidden_list = [[] for _ in range(len(Sf))]
    for f1 in F_plus:
        fl = []
        terms = f1.split('=')
        for term in terms:
            for f in F_minus:
                s = f.split('!=')
                if term == s[0]:
                    forbidden_list[Sf.index(term)].append(Sf.index(s[1]))
                elif term == s[1]:
                    forbidden_list[Sf.index(term)].append(Sf.index(s[0]))

    for i, term in enumerate(Sf):
        
        parents = getParents(i,relations)
        node = Node(id= i,fn = term[0], args= relations[i],find = i, ccpar=parents, fl=forbidden_list[i])
        DAG.append(node)
...
def congruenceClosureAlgorithm(F_plus, F_minus,Sf,new_dag):
    ...
    for f in F_plus:
        ...
        # s = f.split("=")
        # for f2 in F_minus:
        #     if s[0] == f2.split("!=")[0] and s[1] == f2.split("!=")[1]:
        #         return False
...
def start_program(F):
    ...
    dag = createDAG(Sf,relations,F_plus,F_minus)
    print("The DAG is: ")
    for node in dag:
        print(node)
